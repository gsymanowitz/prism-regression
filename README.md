...
- **Automatic transformation discovery** — tests 7 parametric types by default (linear, log, sqrt, square, cubic, inverse, exponential), with an optional extended library of 15 transforms for non-standard functional forms
...
1. **Step 1 — Transformation Screening:** For each predictor, test all transformations (7 standard or 15 extended) against the original response. Rank variables by univariate R².
...
PRISMRegressor(
    ...
    transform_library='standard' # 'standard' (7) or 'extended' (15)
)
```
...
| Method | Description |
|---|---|
| `.fit(X, y)` | Fit the model. Handles dirty data automatically. Pass `dataset_name='...'` to label the PRISM chart. |
...
| `.plot_prism_chart()` | Generate the signature PRISM waterfall chart with variance attribution. |
...
PRISM offers two transformation libraries, selected via the `transform_library` parameter.

### Standard (default, 7 transforms)

The standard library used in the paper. Recommended for most use cases.

| Transform | Formula | Captures |
|---|---|---|
| Linear | x | Proportional effects |
| Logarithmic | sign(x)·log(1+\|x\|) | Diminishing returns |
| Square Root | sign(x)·√\|x\| | Moderate diminishing returns |
...
Logarithmic and Square Root use sign-preserving forms to correctly handle predictors with negative values.

### Extended (15 transforms)

Adds 8 transforms for datasets with saturating, bell-shaped, or hump-shaped relationships. Activate with:

```python
model = PRISMRegressor(m=10, transform_library='extended')
```

The extended library includes all 7 standard transforms plus:

| Transform | Formula | Captures |
|---|---|---|
| Asinh | arcsinh(x) | Compression (like log, but handles negatives natively) |
| Quartic | x⁴ | Strong non-linear acceleration |
| Absolute | \|x\| | Symmetric distance from zero |
| Tanh | tanh(x) | Saturating effects (bounded ±1) |
| Softsign | x/(1+\|x\|) | Saturating effects (lighter tails than tanh) |
| Arctan | arctan(x) | Saturating effects (bounded ±π/2) |
| HumpDecay | x/(1+x²) | Rises then falls (hump-shaped) |
| BellCurve | 1/(1+x²) | Symmetric peak at zero (optimal-point effects) |

**When to use extended:** The extended library is most useful when you suspect non-standard functional forms — for example, variables with an optimal range (bell-shaped) or saturating effects. In benchmarks, the extended library improved test R² by +1.7pp on Boston Housing (76.99% vs 75.31%) where it discovered bell-shaped effects for tax rates and pupil-teacher ratios. On California Housing and Concrete Strength, results were essentially unchanged, confirming the standard 7 are well-chosen for general use.

**Trade-off:** Roughly 1.5–2× slower due to the larger search space, with a slightly higher risk of overfitting on small datasets (n < 500).
